...
class SaveGameSchemeUseCase @Inject constructor(
    private val repository: GameSchemeRepository
) {
    suspend operator fun invoke(gameSchemeData: GameSchemeData) =
        repository.saveGameScheme(gameSchemeData = gameSchemeData)
}